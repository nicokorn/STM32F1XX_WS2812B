...
A Timer is used in which 3 DMA transfers are triggered used to 
write data to the gpios on which the stripes are connected to.
These 3 DMA transfers are triggered as following:
First trigger is on each timer update event. It sets all gpios to high.
...
all gpios always to 0 through a dma transfer. It doesn't matter
